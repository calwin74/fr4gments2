 ...
 */
var move_active = false; // move command clicked
...
//Start delta updates
//timer_delta = setInterval(function(){deltaMapFunction()}, interval_delta);
   ...
   if (request_delta.readyState == 4) {
      if (request_delta.status == 200) {
         ...
         //alert(response);
	      ...
	      //Set army in mapDB and DOM.
         var units = delta.units;
         ...
         //Reset path mark in mapDB and DOM.
         mapDB( {marked:{is:"marked_path"}}).update({marked:null});
...
function updateBoard(records) {
   ...
   records.each(function (r){
      ...
      if (edit) {
	 var d_class = class_coord + " " + toxic_class + " " + c + " " + "edit";
      }
      else {
...
function updateRecordTerrain(x, y, value) {
   var r = getRecord(x, y);
...
function showDevView(id_x, id_y, core_id) {
   ...
   var command_str = null;
   if (move_active) {
      command_str = "command:move";
   }
   document.getElementById('active_command').innerHTML = command_str;
}
...
function handleArmyMovement(core_id) {
   ...
   else if (army_selected || move_active) {
      //get road map
...
function updateCounter(id, sec) {
   document.getElementById(id).innerHTML = sec;
}

function setMoveTimer(id) {
   var d_id = "d" + id;

   var myCounter = new Countdown({  
      seconds:5,  // number of seconds to count down
      updateID:d_id, //id to update
...
$(function() {
   ...
   $(".edit").live("click", function() {
	 ...
	 var coord = xy[0] + "_" + xy[1];
         ...
         updateRecordTerrain(xy[0], xy[1], rand_terrain) {
      }
   });
   ...
   socket.on('connect', function() {
       socket.on('serverHello', function () {
	   alert('serverHello');
